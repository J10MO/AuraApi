...
//         // Update stock and set in_stock to false if stock reaches 0
//         await client.query(
//           `UPDATE products 
...
const orderController = {
  // Create order
  async createOrder(req, res) {
    const { delivery_address, delivery_phone, delivery_name, notes, create_delivery } = req.body
    const client = await pool.connect()
    ...
    try {
      ...
      const deliveryPriceResult = await client.query("SELECT value FROM settings WHERE key = 'delivery_price'")
      const deliveryPrice =
        deliveryPriceResult.rows.length > 0 ? Number.parseFloat(deliveryPriceResult.rows[0].value) : 5.0 // Default fallback
      ...
      const orderResult = await client.query(
        `INSERT INTO orders (order_number, user_id, total_amount, shipping_cost, delivery_address, delivery_phone, delivery_name, notes)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         RETURNING *`,
        [orderNumber, req.user.id, finalTotal, deliveryPrice, delivery_address, delivery_phone, delivery_name, notes],
      )
      ...
      if (create_delivery) {
        ...
        const deliveryResult = await client.query(
          `INSERT INTO deliveries (
            ...
            tracking_number, delivery_status, delivery_fee
          ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
          RETURNING *`,
          [
            order.id,
