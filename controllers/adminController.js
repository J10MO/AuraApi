...
//     try {
//       let query = `SELECT id, name, phone, email, role, membership_level, points, total_orders, is_verified, address_street, address_city, address_district, address_postal_code, created_at, updated_at FROM users WHERE 1=1`
//       let countQuery = `SELECT COUNT(*) FROM users WHERE 1=1`
//       const params = []
//       const countParams = []
//       let paramCount = 0
...
//     try {
//       const result = await pool.query(
//         `SELECT id, name, phone, email, role, membership_level, points, total_orders, is_verified, address_street, address_city, address_district, address_postal_code, created_at, updated_at FROM users WHERE id = $1`,
//         [userId],
//       )
...
//       // Users by membership level
//       const usersByMembership = await pool.query(
//         `SELECT membership_level, COUNT(*) as count FROM users WHERE membership_level IS NOT NULL GROUP BY membership_level`,
//       )
...
const adminUsersController = {
  ...
  async updateUser(req, res) {
    ...
    console.log("[v0] Update user request:", { userId, body: req.body })
    ...
    try {
      ...
      if (role !== undefined) {
        const validRoles = ["customer", "admin", "manager"]
        if (!validRoles.includes(role)) {
          console.log("[v0] Invalid role provided:", role)
          return res.status(400).json({
            success: false,
            error: "Invalid role. Must be: customer, admin, or manager",
            message_ar: "الدور غير صالح. يجب أن يكون: customer أو admin أو manager",
          })
        }
      }

      // Validate membership level if provided
      if (membership_level !== undefined && membership_level !== null) {
        const validLevels = ["bronze", "silver", "gold", "platinum"]
        if (!validLevels.includes(membership_level)) {
          console.log("[v0] Invalid membership level provided:", membership_level)
          return res.status(400).json({
            success: false,
            error: "Invalid membership level. Must be: bronze, silver, gold, or platinum",
            message_ar: "مستوى العضوية غير صالح. يجب أن يكون: bronze أو silver أو gold أو platinum",
          })
        }
      }
  ...
  async deleteUser(req, res) {
    ...
    console.log("[v0] Delete user request:", { userId, requestingAdmin: req.user.id })

    try {
      // Check if user exists
      const userCheck = await pool.query("SELECT id, role, name, phone FROM users WHERE id = $1", [userId])
      if (userCheck.rows.length === 0) {
        return res.status(404).json({
          success: false,
          error: "User not found",
