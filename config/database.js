...
const pool = new Pool({
  ...
  max: 1000, // Limit connections in serverless
  idleTimeoutMillis: 10000,
  connectionTimeoutMillis: 5000,
})
...
async function initDatabase() {
  try {
    ...
    await pool.query(`
      ...
      VALUES ('delivery_price', '5.00', 'Default delivery price for orders')
    ...
    // Create indexes for deliveries table for better query performance
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_deliveries_order_id 
      ON deliveries(order_id)`)

    await pool.query(`CREATE INDEX IF NOT EXISTS idx_deliveries_user_id 
      ON deliveries(user_id)`)

    await pool.query(`CREATE INDEX IF NOT EXISTS idx_deliveries_status 
      ON deliveries(delivery_status)`)

    await pool.query(`CREATE INDEX IF NOT EXISTS idx_deliveries_tracking 
